...
public class Task extends elv.util.Propertable implements java.io.Serializable, elv.util.Visualizable
{
  ...
  {
    return parse(new Container(elv.util.User.parse(elv.util.server.ServerStub.getRoot(),
      relativePath.split(java.util.regex.Pattern.quote("/"))[0]),
      relativePath.split(java.util.regex.Pattern.quote("/"))[1]),
      relativePath.split(java.util.regex.Pattern.quote("/"))[2]);
  }
  ...
  public void loadProperties() throws java.lang.Exception
  {
    ...
    {
      properties = elv.util.Property.load(container.getFolderPath(), name + "/" + PROPERTY_FOLDER + "/" + PROPERTY_FILE_NAME);
    }
    else
    {
      try
  ...
  {
    elv.util.Property.store(getPropertyFolderPath() + "/" + PROPERTY_FILE_NAME, properties);
  }
  ...
  {
    return container.getFolderPath() + "/" + name;
  }
  ...
  {
    return getFolderPath() + "/" + PROPERTY_FOLDER;
  }
  ...
  {
    return getFolderPath() + "/" + EXECUTION_FOLDER;
  }
  ...
  public synchronized void create() throws java.lang.Exception
  {
    ...
    if(!done)
    {
      throw new java.io.IOException("Cannot create " + getFolderPath());
    }
    // Create task properties folder.
    done = new java.io.File(getPropertyFolderPath()).mkdir();
    if(!done)
    {
      throw new java.io.IOException("Cannot create " + getPropertyFolderPath());
    }
    // Create task execution folder.
    done = new java.io.File(getExecutionFolderPath()).mkdir();
    if(!done)
    {
      throw new java.io.IOException("Cannot create " + getExecutionFolderPath());
    }
  ...
  public synchronized void move(elv.task.Task newTask) throws java.lang.Exception
  {
    ...
    else
    {
      ...
      {
        java.lang.String newFolderPath = newTask.getFolderPath();
        if(!newTask.getContainer().equals(getContainer()))
        {
          ...
          newFolderPath = newTask.getContainer().getFolderPath() + "/" + newName;
        }
  ...
  public synchronized void copy(Task newTask) throws java.lang.Exception
  {
    ...
    else
    {
      ...
      {
        java.lang.String newName = elv.util.Util.getNumbered(newTask.getName(), elv.util.Util.vectorize(new java.io.File(newTask.getContainer().getFolderPath()).list()), null);
        java.lang.String newFolderPath = newTask.getContainer().getFolderPath() + "/" + newName;
  ...
  public synchronized void clean() throws java.lang.Exception
  {
    ...
    {
      elv.util.Util.delete(path + "/" + paths[i]);
    }
  }
  ...
  {
    return container.getUser() + "/" + container + "/" + name;
  }
